...
QSvgNode *QSvgTinyDocument::elementOnPosition(const QPoint &mouseposition)
{
   for(int i = m_renderers.size()-1; i > 0; --i)
   {
      QSvgNode *node = m_renderers.at(i);
      if ((node->isVisible()) && (node->displayMode() != QSvgNode::NoneMode))
      {
         if(node->hitTest(mouseposition))
         {
//            qDebug() << " Node:" << node->nodeId() << " pressed";
            return node;
         }
      }
   }
...
QSvgNode *QSvgTinyDocument::elementWithActionOnPosition(const QPoint &mouseposition)
{
   if(m_renderers.isEmpty())
      return 0L;

   QSvgNode* test = 0L;
   for(int i = m_renderers.size()-1; i > -1; --i)
   {
      QSvgNode *node = m_renderers.at(i);
      QSvgNode* test = findActionNode(node, mouseposition);
      if(test != 0L)
         return test;
   }
   return test;
...
QSvgNode* QSvgTinyDocument::findActionNode(QSvgNode* node, const QPoint& mouseposition)
{
   if ((node->isVisible()) && (node->displayMode() != QSvgNode::NoneMode))
   {
      if(node->hasAction() && node->isEnabled())
      {
         if(node->hitTest(mouseposition))
         {
            return node;
      ...
      if(node->type() == QSvgNode::G)
      {
         // User clicked on group
         // The group is visible but does not have an action.
         // Maybe one of the children has an action?
         QSvgStructureNode* group = static_cast<QSvgStructureNode*>(node);
         QList<QSvgNode*> nodelist = group->renderers();

         if(!nodelist.isEmpty())
         {
            for(int i = nodelist.size()-1; i > -1; --i)
            {
               QSvgNode* ret = findActionNode(nodelist.at(i), mouseposition);
               if(ret)
                  return ret;
            }
         }
...
QSvgNode *QSvgTinyDocument::elementWithTooltipOnPosition(const QPoint &mouseposition)
{
   if(m_renderers.isEmpty())
      return 0L;
   if(m_renderers.isEmpty())
   ...
   {
      QSvgNode *node = m_renderers.at(i);
      QSvgNode* test = findTooltipNode(node, mouseposition);
      if(test != 0L)
         return test;
   }
   return test;
...
QSvgNode* QSvgTinyDocument::findTooltipNode(QSvgNode* node, const QPoint& mouseposition)
{
   if ((node->isVisible()) && (node->displayMode() != QSvgNode::NoneMode))
   {
      if(node->hasTooltip())
      ...
      if(node->type() == QSvgNode::G)
      {
         ...
         QSvgStructureNode* group = static_cast<QSvgStructureNode*>(node);
         QList<QSvgNode*> nodelist = group->renderers();
         //         QString groupname = node->nodeId();

         if(!nodelist.isEmpty())
         {
            for(int i = nodelist.size()-1; i > -1; --i)
            {
               QSvgNode* ret = findTooltipNode(nodelist.at(i), mouseposition);
               if(ret)
                  return ret;
            }
         }
