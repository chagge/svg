...
QSvgAttributes::QSvgAttributes(const QXmlStreamAttributes &xmlAttributes, QSvgHandler *handler)
{
   ...
   for (int i = 0; i < xmlAttributes.count(); ++i) {
      ...
      switch (name.at(0).unicode())
      {
         ...
         case 'm':
            if (name == QLatin1String("movefrom"))
            {
               movefrom = value;
            }
            else if (name == QLatin1String("moveto"))
            {
               moveto = value;
            }
            break;
         ...
         case 't':
            if (name == QLatin1String("text-anchor"))
               ...
               transform = value;
            else if (name == QLatin1String("tooltipsource"))
               tooltip = value;
            break;
...
public:
   ...
   {
      return static_cast<QSvgNode*>/*(QSvgNode*)*/(node.ptr);
   }
   inline QSvgStructureNode *nodeToStructure(QSvgNode *n) const
   {
      if (n &&
         ...
         return static_cast<QSvgStructureNode*>(n);
      }
      return 0;
   }
...
{
   // DF added these
   node->setAction(!attributes.actions.isEmpty());
   node->setTooltip(!attributes.tooltip.isEmpty());
   node->setTooltipText(attributes.tooltip.toString());

   qreal x, y;
   int i = attributes.movefrom.toString().indexOf(',');
   if( i > 0)
   {
      x = attributes.movefrom.toString().section(',', 0, 0).toDouble();
      y = attributes.movefrom.toString().section(',', 1, 1).toDouble();
      node->setMoveFrom(QPointF(x,y));
   }
   i = attributes.moveto.toString().indexOf(',');
   if( i > 0)
   {
      x = attributes.moveto.toString().section(',', 0, 0).toDouble();
      y = attributes.moveto.toString().section(',', 1, 1).toDouble();
      node->setMoveTo(QPointF(x,y));
   }
   // DF end
                                 ...
                                 QSvgHandler *handler)
{
   ...
   else if(filename.startsWith(QLatin1String("cache:")))
   {
      QString cachefile = filename.mid(6);
      QNetworkDiskCache diskCache;
      diskCache.setCacheDirectory(QDesktopServices::storageLocation(QDesktopServices::CacheLocation));
      QIODevice *f = (QFile *)diskCache.data(QUrl::fromLocalFile(cachefile.toUpper()));
      if(f)
      {
