...
bool QSvgEllipse::hitTest(const QPoint &position)
{
   return m_hittest.contains(position);
...
void QSvgEllipse::draw(QPainter *p, QSvgExtraStates &states)
{
   ...
   if(m_fillFactor < 1.0)
   {
      cliprect = setMyClip(m_bounds, m_fillFactor, m_fillDirection);
      p->setClipRect(cliprect, Qt::IntersectClip);
   }

   qreal offsetx = 0.0;
   qreal offsety = 0.0;
   QRectF moved(m_bounds);
   if(m_moveFactor != 1.1)
   {
      // Recalculate the position of the path
      QPointF orgcenter = moved.center();
      QPointF a = getMoveFrom();

      offsetx = orgcenter.x()-a.x();
      offsety = orgcenter.y()-a.y();

      QPointF b = getMoveTo();
      offsetx += (b.x() - a.x()) * m_moveFactor;
      offsety -= (b.y() - a.y()) * m_moveFactor;

      p->translate(QPointF(offsetx, -offsety));
   }

   doRotate(p, m_bounds.center());

   QT_SVG_DRAW_SHAPE(p->drawEllipse(m_bounds));

   cleanRotate(p, m_bounds.center());

   if(m_showAction)
   {
      p->setWorldTransform(m_trans);
   }
   else
   {
      QPainterPath path;
      path.addEllipse(m_bounds);
      m_hittest = p->worldTransform().map(path);
   }
...
void QSvgLine::draw(QPainter *p, QSvgExtraStates &states)
{
   ...
   {
      centerx =  m_line.x2() - m_line.x1();
   }
   else
   {
      centerx =  m_line.x1() - m_line.x2();
   }

   if(m_line.y1() < m_line.y2())
   {
      centerx =  m_line.y2() - m_line.y1();
   }
   else
   {
      centerx =  m_line.y1() - m_line.y2();
   }
   doStrokeFill(p, m_line.length());
   doRotate(p, QPointF(centerx, centery));
   ...
   cleanRotate(p, QPointF(centerx, centery));
   revertStyle(p, states);
}
...
void QSvgPath::draw(QPainter *p, QSvgExtraStates &states)
{
   ...
   if(m_fillFactor < 1.0)
   {
      cliprect = setMyClip(m_rect, m_fillFactor, m_fillDirection);
      p->setClipRect(cliprect, Qt::IntersectClip);
   }
   ...
   doStrokeFill(p, m_path.length());

   if(m_moveFactor < 1.0)
   {
      // Recalculate the position of the path
      QPointF orgcenter = m_path.boundingRect().center();
      QPointF a = getMoveFrom();
   ...
   doRotate(p, m_path.boundingRect().center());

   QT_SVG_DRAW_SHAPE(p->drawPath(m_path));

   cleanRotate(p, m_path.boundingRect().center());
   ...
   {
      m_hittest = p->worldTransform().map(m_path);
      m_hitRect = m_hittest.boundingRect();
   }
   if(m_fillFactor < 1.0)
   {
      p->setClipRect(cliprect, Qt::NoClip);
...
bool QSvgPath::hitTest(const QPoint &position)
{
   return m_hittest.contains(position);
}
...
bool QSvgPolygon::hitTest(const QPoint &position)
{
   return m_hittest.containsPoint(position, Qt::WindingFill);
}
...
void QSvgPolygon::draw(QPainter *p, QSvgExtraStates &states)
{
   ...
   if(m_factor < 1.0)
   {
      cliprect = setMyClip(m_rect, m_factor, m_direction);
      p->setClipRect(cliprect, Qt::IntersectClip);
   }
   ...
   {
      // Recalculate the position of the path
      QPointF orgcenter = m_poly.boundingRect().center();
      QPointF a = getMoveFrom();
   ...
   doRotate(p, m_poly.boundingRect().center());

   QT_SVG_DRAW_SHAPE(p->drawPolygon(m_poly, states.fillRule));

   cleanRotate(p, m_poly.boundingRect().center());
   ...
   {
      m_hittest = p->worldTransform().map(m_poly);
      m_hitRect = m_hittest.boundingRect();
   }
   if(m_factor < 1.0)
   {
...
bool QSvgRect::hitTest(const QPoint &position)
{
   return m_hitRect.contains(position);
}
...
void QSvgRect::draw(QPainter *p, QSvgExtraStates &states)
{
   ...
   if(m_fillfactor < 1.0)
   {
      cliprect = setMyClip(m_rect, m_fillfactor, m_direction);
      p->setClipRect(cliprect, Qt::IntersectClip);
   }
   ...
   {
      // Recalculate the position of the path
      QPointF orgcenter = m_rect.center();
      QPointF a = getMoveFrom();
   ...
   {
      m_hitRect = p->worldTransform().mapRect(m_rect);
      m_test = p->worldTransform().mapRect(m_rect);
   }
   if(m_fillfactor < 1.0)
   {
      p->setClipRect(cliprect, Qt::NoClip);
...
void QSvgText::draw(QPainter *p, QSvgExtraStates &states)
{
   ...
   for (int i = 0; i < m_tspans.size(); ++i) {
      if (m_tspans[i] == LINEBREAK) {
         ...
         // DF trim de tspans waar '\n' in staat!!
         if(newText.contains(QLatin1Char('\n')))
         {
            newText.clear();
         }
         // Orginele code hieronder.
         /*
...
*/
   ...
   if (states.svgFont) {
      ...
      for (int i = 0; i < paragraphs.size(); ++i) {
         ...
         //QString a = tl.text().trimmed();
         //tl.setText(a);
         m_trans = p->worldTransform();
         tl.draw(p, QPointF(px, py), QVector<QTextLayout::FormatRange>(), bounds);
         //double xs = px/scale;
         //double ys = py/scale;
         m_bounds = tl.boundingRect();

         //double xd = m_bounds.right();
         //double yd = m_bounds.bottom();

         m_bounds.setTop(py);
         m_bounds.setRight(px);

         m_bounds = p->worldTransform().mapRect(m_bounds);
...
QRectF QSvgText::getBounds(void)
{
   return m_bounds;
}
...
// DF added this
void QSvgText::setText(const QString &text)
{
   int i = m_tspans.size();
   if(m_tspans.size() == 1)
   {
      m_tspans.at(0)->setText(text);
   }
   else if(m_tspans.size() == 3)
   {
      m_tspans.at(1)->setText(text);
   }
   else if(m_tspans.size() == 5) // If description has been filled in
   {
      m_tspans.at(3)->setText(text);
   }
}

bool QSvgText::hitTest(const QPoint &position)
{
   return false;
...
QRectF setMyClip(const QRectF& rect, qreal factor, int direction)
{
   ...
   {
      case 0:
         delta = rect.height() - (rect.height() * factor);
         cliprect.adjust(0.0,
                         delta,
                         0.0,
                         0.0);
      ...
      case 1:
         ...
         break;
      case 2:
         delta = rect.width() - (rect.width() * factor);
         cliprect.adjust(0.0,
                         0.0,
                         -delta,
                         0.0);
